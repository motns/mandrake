  ...
  context "::validate" do
    ...
    context "called with Range as the :in parameter" do
      context "called with valid integer" do
        it "returns true" do
          described_class.send(:validate, 5, in: 0..10).should be_true
        end
      ...
      context "called with a invalid integer" do
        it "returns false" do
          described_class.send(:validate, 15, in: 0..10).should be_false
        end

        it "sets the error code to :not_in_range" do
          described_class.send(:last_error_code).should eq(:not_in_range)
        end

        it "sets the error message for :not_in_range" do
          described_class.send(:last_error).should eq("must be between 0 and 10")
        end
      end
    end
    ...
    context "called with Array as the :in parameter" do
      context "called with valid string" do
        it "returns true" do
          described_class.send(:validate, 'two', in: %w(one two three)).should be_true
        end
      ...
      context "called with an invalid string" do
        it "returns false" do
          described_class.send(:validate, 'four', in: %w(one two three)).should be_false
        end

        it "sets the error code to :not_in_set" do
          described_class.send(:last_error_code).should eq(:not_in_set)
        end

        it "sets the error message for :not_in_set" do
          described_class.send(:last_error).should eq("must be one of: one, two, three")
        end
      end
    end
    ...
    context "called with Date as the :in parameter" do
      context "called with valid date" do
        it "returns true" do
          described_class.send(:validate, 2.days.ago.to_date, in: 1.week.ago.to_date..Date.today).should be_true
        end
      ...
      context "called with an invalid date" do
        it "returns false" do
          described_class.send(:validate, 2.weeks.ago, in: 1.week.ago.to_date..Date.today).should be_false
        end
        ...
        it "sets the error message for :not_in_range" do
          described_class.send(:last_error).should eq("must be between #{1.week.ago.to_date} and #{Date.today}")
        end
      end
    end
